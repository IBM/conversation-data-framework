...
class TestConvertWorkspaceBackAndForth(BaseTestCaseCapture):
    ...
    def test_basic(self, dialogFilename):
        ...
        self.tf(dialog_json2xml.main, [dialogJsonRefPath])
        with open(dialogXmlPath, 'w') as f:
            f.write(self.captured.out)

        # convert xml back to json
        self.tf(dialog_xml2json.main, ['-dm', dialogXmlPath, '-of', self.testOutputPath, '-od', dialogFilename, '-s', self.dialogSchemaPath, '-c', configPath, '-v'])

        # compare dialogs if they are same
        self.tfExitCode(compare_dialogs.main, 0, [dialogJsonRefPath, dialogJsonHypPath])
